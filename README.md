...
| `GET` | `/api/v1/user/info?userId=...` | `UserController.getInfo`, `UserServiceImpl.getUserInfo` | Required | Query `userId: UUID` | `BaseResponse<UserInfoDto>` | Lấy thông tin user theo id. Nếu không phải chính mình, chỉ trả khi hai user là bạn bè. | `userId` phải parse được UUID. | `404 User not found`, `401`, `403`, `500` |
...
| Method | URL | Controller/service | Auth | Request | Response | Mô tả | Validation chính | Error có thể xảy ra |
|---|---|---|---|---|---|---|---|---|
| `GET` | `/api/v1/friend` | `FriendController.getListFriend`, `FriendServiceImpl.getListFriend` | Required | Query `limit`, `cursor`, `search` | `BaseResponse<PaginationResponse<FriendDto>>` | Lấy danh sách bạn bè theo cursor pagination, có filter search. | `limit > 0`; cursor ISO datetime nếu có. | `400 Limit must be greater than 0.`, `400 Cursor is invalid.`, `401`, `403`, `500` |
| `GET` | `/api/v1/friend/search?username=...` | `FriendController.searchByUsername`, `FriendServiceImpl.searchByUsername` | Required | Query `username` optional | `BaseResponse<List<FriendSearchDto>>` | Search user theo username, trả kèm trạng thái quan hệ. | Search rỗng trả list rỗng. | `401`, `403`, `500` |
...
| `PATCH` | `/api/v1/conversation/{conversationId}/group` | `ConversationController.updateGroup`, `ConversationServiceImpl.updateGroup` | Required | Path `conversationId`, body `UpdateGroupRequest` | `BaseResponse<ConversationDto>` | Cập nhật thông tin nhóm (chỉ `name`) cho group conversation. | Active participant trong group; `name` không blank; không áp dụng cho DIRECT. | `404 Group conversation not found.`, `403`, `400 Validation` |
...
| `PaginationResponse<T>` | `nextCursor` | `String` | Optional | Không có | Cursor cho trang tiếp theo. |
...
| `FriendSearchDto` | `id` | `UUID` | Required | Không có | User id. |
| `FriendSearchDto` | `username` | `String` | Required | Không có | Username. |
| `FriendSearchDto` | `firstName` | `String` | Required | Không có | Tên. |
| `FriendSearchDto` | `lastName` | `String` | Required | Không có | Họ. |
| `FriendSearchDto` | `avatarUrl` | `String` | Optional | Không có | Avatar. |
| `FriendSearchDto` | `joinedAt` | `LocalDateTime` | Required | Không có | User created time. |
| `FriendSearchDto` | `status` | `FriendSearchStatus` | Required | Không có | `NONE`, `SELF`, `FRIEND`, `SENT`, `RECEIVED`. |
| `FriendSearchDto` | `requestId` | `UUID` | Optional | Không có | Pending friend request id nếu có. |
...
| `ConversationEvent` | `eventType` | `String` | Required | Static factory set `conversation.updated`. | Loại event. |
...
| `/user/queue/conversations` | `ConversationEvent` | Sau khi gửi message thành công hoặc mark seen thành công. | `src/main/java/com/chat_socket/socket/SocketEmitter.java`, `src/main/java/com/chat_socket/socket/SocketPublisher.java` |
...
### `PaginationRequest(limit, cursor)`
...
| `cursor` | `String` | `null` | Không áp dụng | Cursor datetime. |
...
### `PaginationResponse<T>(messages, nextCursor)`
...
| Participant lifecycle | Entity có `leftAt`, `archivedAt`, `deletedAt`, `mutedUntil`, nhưng source hiện tại chưa thấy API rời nhóm/archive/mute/delete conversation. |
...
| `FriendSearchStatus` | `NONE`, `SELF`, `FRIEND`, `SENT`, `RECEIVED` | `src/main/java/com/chat_socket/enums/FriendSearchStatus.java` |
